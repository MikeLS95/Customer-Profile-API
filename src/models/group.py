...
class GroupSchema(ma.Schema):
    ...
    # Nests user details in group json response
    first_member = fields.Nested(
        'UserSchema', only=['id', 'first_name', 'last_name', 'email'])
    second_member = fields.Nested(
        'UserSchema', only=['id', 'first_name', 'last_name', 'email'])
    third_member = fields.Nested(
        'UserSchema', only=['id', 'first_name', 'last_name', 'email'],
        allow_none=True)
    fourth_member = fields.Nested(
        'UserSchema', only=['id', 'first_name', 'last_name', 'email'],
        allow_none=True)

    class Meta:
        fields = ('id', 'name', 'first_member_id', 'second_member_id',
                  'third_member_id', 'fourth_member_id', 'first_member',
                  'second_member', 'third_member', 'fourth_member')
